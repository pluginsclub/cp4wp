...
// DB PAGE
function pluginsclub_cpanel_postgresql_page() {
...
<div id="pluginsclub-cpanel-header-title-nav">
    ...
    ?>
			...
			<h1 class="pluginsclub-cpanel-hide">cPanel PostgreSQL</h1>
				<h2>PostgreSQL Databases</h2>
        <p>cPanel PostgreSQL Databases</p>
...
<div id="create-user-form-container" style="display:none;">
    <h3>Create a PostgreSQL User</h3>
    <table class="form-table" role="presentation">
...
<div id="change-user-form-container" style="display:none;">
    <h3>Change Password for a PostgreSQL User</h3>
    <table class="form-table" role="presentation">


<?php
// Set up API request parameters
        $api_url = "https://$hostname:2083/cpsess1235467/execute/Postgresql/list_users";
        $args = array(
            'headers' => array(
...
/////// LIST DATABASES
// Set up API request parameters
$api_url2 = "https://$hostname:2083/cpsess1235467/execute/Postgresql/list_databases";
$args2 = array(
    'headers' => array(
        'Authorization' => 'Basic ' . base64_encode( "$username:$password" ),
        'Content-Type' => 'application/json'
...
<h3>Assign a User to PostgreSQL Database</h3>
<table class="form-table" role="presentation">
...
<div id="create-remove-form-container" style="display:none;">
    <h3>Remove a user from PostgreSQL Database</h3>
        <table class="form-table" role="presentation">
...
<?php $phppgadmin_link = "https://" . $hostname . ":2083/login/?user=" . $username . "&pass=" . $password . "&goto_uri=/3rdparty/phpPgAdmin/index.php" ?>
<a href="<?php echo $phppgadmin_link ?>" target="_blank" class="button" id="phppgadmin"><img src="<?php echo plugin_dir_url( __FILE__ ) . 'icons/phppgadmin_logo.png'; ?>" style="vertical-align: middle;" alt=""></a>
...
if (isset($_POST['create_db'])) {
    ...
    $url = "https://$hostname:2083/cpsess1235467/execute/Postgresql/create_database?name=" . urlencode($username) . '_' . urlencode($create_db);
    ...
    if ($data['status'] == 1) {
             $messagecreate = "<div class='notice notice-success is-dismissible'><p>Successfully created a new PostgreSQL database <b>$create_db</b>.</p></div>";


    } else {
            $messagecreate = "<div class='notice notice-error is-dismissible'><p>Error creating new PostgreSQL database <b>$create_db</b>: </br>" . implode(", ", $data['errors']) . "</p></div>";
...
if (isset($_POST['create_user']) && isset($_POST['create_password'])) {
    ...
    $url = "https://$hostname:2083/cpsess1235467/execute/Postgresql/create_user?name=" . urlencode($username) . '_' . urlencode($create_user) . '&password=' . urlencode($create_password);
    $response = wp_remote_get($url, array(
        'headers' => array(
            'Authorization' => 'Basic ' . base64_encode(get_option('cpanel_username') . ':' . get_option('cpanel_password'))
        )
    ...
    if ($data['status'] == 1) {
             $messagecreateuser = "<div class='notice notice-success is-dismissible'><p>Successfully created a new PostgreSQL user <b>$create_user</b>.</p></div>";


    } else {
            $messagecreateuser = "<div class='notice notice-error is-dismissible'><p>Error creating new PostgreSQL user <b>$create_user</b>: </br>" . implode(", ", $data['errors']) . "</p></div>";
...
if (isset($_POST['change_user']) && isset($_POST['change_password'])) {
    ...
    // Build the URL with the extracted values
    $url = "https://$hostname:2083/cpsess1235467/execute/Postgresql/set_password?user=" . urlencode($change_user) . '&password=' . urlencode($change_password);
    $response = wp_remote_get($url, array(
        'headers' => array(
            'Authorization' => 'Basic ' . base64_encode(get_option('cpanel_username') . ':' . get_option('cpanel_password'))
        )
...
if (isset($_POST['assign_user']) && isset($_POST['assign_db'])) {
    ...
    // Build the URL with the extracted values
    $url = "https://$hostname:2083/cpsess1235467/execute/Postgresql/grant_all_privileges?user=" . urlencode($assign_user) . urlencode($create_user) . '&database=' . urlencode($assign_db);
    $response = wp_remote_get($url, array(
        'headers' => array(
            'Authorization' => 'Basic ' . base64_encode(get_option('cpanel_username') . ':' . get_option('cpanel_password'))
        )
    ...
    if ($data['status'] == 1) {
             $messagecreatepriv = "<div class='notice notice-success is-dismissible'><p>Successfully added PostgreSQL user <b>$assign_user</b> to the database <b>$assign_db</b> and grant ALL privileges.</p></div>";


    } else {
            $messagecreatepriv = "<div class='notice notice-error is-dismissible'><p>Error assigning PostgreSQL user <b>$assign_user</b> to the database <b>$assign_db</b>: </br>" . implode(", ", $data['errors']) . "</p></div>";
...
if (isset($_POST['remove_user']) && isset($_POST['remove_db'])) {
    ...
    // Build the URL with the extracted values
    $url = "https://$hostname:2083/cpsess1235467/execute/Postgresql/revoke_all_privileges?user=" . urlencode($assign_user) . urlencode($remove_user) . '&database=' . urlencode($remove_db);
    $response = wp_remote_get($url, array(
        'headers' => array(
            'Authorization' => 'Basic ' . base64_encode(get_option('cpanel_username') . ':' . get_option('cpanel_password'))
        )
    ...
    if ($data['status'] == 1) {
             $messageremove = "<div class='notice notice-success is-dismissible'><p>Successfully removed PostgreSQL user <b>$assign_user</b> from the database <b>$assign_db</b> and revoke ALL privileges.</p></div>";


    } else {
            $messageremove = "<div class='notice notice-error is-dismissible'><p>Error removing PostgreSQL user <b>$assign_user</b> from the database <b>$assign_db</b>: </br>" . implode(", ", $data['errors']) . "</p></div>";
...
$api_url = "https://$hostname:2083/cpsess1235467/execute/Postgresql/list_databases";
$args = array(
    'headers' => array(
        'Authorization' => 'Basic ' . base64_encode( "$username:$password" ),
        'Content-Type' => 'application/json'
...
if ( $data && isset( $data['data'] ) && count( $data['data'] ) > 0 ) {
    ...
    <table class="wp-list-table widefat fixed striped" id="baze">
        <thead>
            <tr>
                <th>Database</th>
                <th>Users</th>
    ...
    <?php
            ...
            echo '<p>No PostgreSQL databases found.</p>';
        }
